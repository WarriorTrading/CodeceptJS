...
module.exports = (text, file) => {
  ...
  for (const featureChild of ast.feature.children) {
    const child = Object.values(featureChild)[0];
    if (child == null) {
      continue;
    }
    if (child.keyword === 'Rule') {
      console.log('*** rule:', child);
    }
    if (child.keyword === 'Background') {
      suite.beforeEach('Before', scenario.injected(async () => runSteps(child.steps), suite, 'before'));
      continue;
    }
    if (child.keyword === 'Scenario' && child.examples.length > 0) {
